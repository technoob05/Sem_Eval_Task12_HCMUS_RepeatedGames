...
"""
================================================================================
Exp38: RAG-FUSION - Multi-Query Retrieval
================================================================================
SELF-CONTAINED - Copy to Kaggle notebook

🎯 KEY INSIGHT: Single query may miss relevant evidence
📊 Different query formulations capture different aspects  
✅ SOLUTION: RAG-Fusion - multiple query variants, retrieve, fuse with RRF

Query Variants:
1. Event-only query
2-5. Event + each Option (A/B/C/D)
6. Combined query

Based on: RAG-Fusion (arXiv:2402.03367)

Includes: Hybrid RAG + Pooling fix
...
class Config:
    ...
    if IS_KAGGLE:
        ...
        OUTPUT_DIR = Path('/kaggle/working/exp38_rag_fusion_output')
    else:
        DATA_DIR = Path(r'd:\Work\SemEval 2026 Task 12 - Abductive Event Reasoning\semeval2026-task12-dataset')
        OUTPUT_DIR = Path(r'd:\Work\SemEval 2026 Task 12 - Abductive Event Reasoning\exp38_rag_fusion_output')
...
print(f"{'='*70}")
print(f"Exp38: RAG-FUSION - Multi-Query Retrieval")
print(f"{'='*70}")
print(f"✅ 6 query variants for comprehensive retrieval")
print(f"✅ Hybrid BM25 + Dense per query")
print(f"✅ Cross-query RRF fusion")
...
# ============================================================================
# ⭐ RAG-FUSION RETRIEVER - Multi-Query with Cross-Query RRF
# ============================================================================
class RAGFusionRetriever:
    ...
    def _generate_query_variants(self, event, options):
        """Generate 6 query variants for RAG-Fusion"""
        return [
            event,  # Event-only
            f"{event} {options[0][:80]}",  # Event + Option A
            f"{event} {options[1][:80]}",  # Event + Option B
            f"{event} {options[2][:80]}",  # Event + Option C
            f"{event} {options[3][:80]}",  # Event + Option D
            f"{event} " + " ".join([opt[:40] for opt in options]),  # Combined
        ]
    
    def _get_hybrid_ranks(self, query, chunks, tokenized_chunks, chunk_meta, graph):
        """Get hybrid BM25+Dense ranks for a single query"""
        try:
