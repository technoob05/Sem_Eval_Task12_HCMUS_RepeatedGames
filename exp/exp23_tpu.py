...
"""
================================================================================
Exp23-TPU: CF-RAG - Counterfactual RAG (TPU Optimized)
================================================================================
SELF-CONTAINED - Copy to Kaggle TPU notebook

🔥 TPU OPTIMIZATIONS:
1. torch_xla for TPU acceleration
2. Larger model: DeBERTa-v2-xlarge (900M params)
3. No AMP/GradScaler
4. XLA optimizer step
...
import json, random, shutil, warnings, os, re
import numpy as np
...
class Config:
    ...
    if IS_KAGGLE:
        ...
        OUTPUT_DIR = Path('/kaggle/working/exp23_tpu_output')
    else:
        DATA_DIR = Path(r'd:\Work\SemEval 2026 Task 12 - Abductive Event Reasoning\semeval2026-task12-dataset')
        OUTPUT_DIR = Path(r'd:\Work\SemEval 2026 Task 12 - Abductive Event Reasoning\exp23_tpu_output')
    ...
    # CF-RAG Settings
    USE_COUNTERFACTUAL_QUERY = True
    USE_CONTRASTIVE_EVIDENCE = True
    CF_AUGMENT_PROB = 0.15
    NEGATION_PATTERNS = [
...
print(f"{'='*70}")
print(f"Exp23-TPU: CF-RAG - Counterfactual RAG")
print(f"{'='*70}")
    ...
    def generate_counterfactual(self, text):
        ...
        for original, negated in self.patterns:
            if original in text.lower():
                cf_text = re.sub(re.escape(original), negated, text, count=1, flags=re.IGNORECASE)
                break
        if cf_text == text:
...
class ContrastiveEvidenceRetriever:
    ...
    def retrieve_contrastive(self, topic_info, event, options, max_chars=1200):
        if not topic_info:
            return ""
        ...
        if config.USE_CONTRASTIVE_EVIDENCE:
            ...
            top_contrast = np.argsort(contrast_score)[-1:][::-1]
...
# ============================================================================
def train_epoch(model, loader, optimizer, scheduler, bce_criterion, cf_criterion):
    model.train()
    total_loss = 0
...
def main():
    ...
    model = CFRAGModel(config.MODEL_NAME).to(DEVICE)
    ...
    for epoch in range(config.EPOCHS):
        ...
        loss = train_epoch(model, train_loader, optimizer, scheduler, bce_criterion, cf_criterion)
    ...
    zip_path = create_submission(test_probs, test_ids, ckpt['threshold'], config.OUTPUT_DIR, 'exp23_tpu_submission')
    
    print(f"\n{'='*70}")
    print(f"🔥 CF-RAG TPU Complete!")
    print(f"  Model: {config.MODEL_NAME}")
    print(f"  Dev: {best_score:.4f}")
