...
"""
================================================================================
Exp40: FULL SOTA - Combined Best Practices
================================================================================
SELF-CONTAINED - Copy to Kaggle notebook

🏆 SOTA COMBINATION:
1. ✅ Fix Pooling: Last non-pad token (exp34)
2. ✅ Label Powerset: 15-class classification for Exact Match (exp35)
3. ✅ RAG-Fusion: Multi-Query + Hybrid + RRF Retrieval (exp38)
4. ✅ Causal Edge Boosting (exp22)

This experiment combines all "high ROI" improvements into a single pipeline.
Label Powerset replaces the need for Margin Policy as it learns joint distributions directly.
...
class Config:
    ...
    if IS_KAGGLE:
        ...
        OUTPUT_DIR = Path('/kaggle/working/exp40_full_sota_output')
    else:
        DATA_DIR = Path(r'd:\Work\SemEval 2026 Task 12 - Abductive Event Reasoning\semeval2026-task12-dataset')
        OUTPUT_DIR = Path(r'd:\Work\SemEval 2026 Task 12 - Abductive Event Reasoning\exp40_full_sota_output')
...
print(f"{'='*70}")
print(f"Exp40: FULL SOTA COMBINATION")
print(f"{'='*70}")
...
def main():
    ...
    zip_path = create_submission(test_answers, test_ids, config.OUTPUT_DIR, 'exp40_full_sota_submission')
    
    with open(config.OUTPUT_DIR/'results.json', 'w') as f:
        json.dump({'experiment': 'exp40_full_sota', 'dev_score': float(best_score), 'model': config.MODEL_NAME}, f, indent=2)
    print(f"\n🏆 Exp40 Complete! SOTA Combination of RAG-Fusion + Label Powerset + Pooling Fix. Submission: {zip_path}")
