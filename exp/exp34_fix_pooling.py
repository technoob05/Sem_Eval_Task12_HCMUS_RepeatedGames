...
"""
================================================================================
Exp34: FIX POOLING - Last Non-Pad Token (Critical Bug Fix)
================================================================================
SELF-CONTAINED - Copy to Kaggle notebook

🎯 FIX: exp22_7b uses hidden_states[:, -1, :] which captures PAD tokens!
🔧 SOLUTION: Use attention_mask to find actual last non-pad token

This is a "FREE SCORE" fix - the model was learning from garbage PAD representations.
...
class Config:
    ...
    if IS_KAGGLE:
        ...
        OUTPUT_DIR = Path('/kaggle/working/exp34_fix_pooling_output')
    else:
        DATA_DIR = Path(r'd:\Work\SemEval 2026 Task 12 - Abductive Event Reasoning\semeval2026-task12-dataset')
        OUTPUT_DIR = Path(r'd:\Work\SemEval 2026 Task 12 - Abductive Event Reasoning\exp34_fix_pooling_output')
...
print(f"{'='*70}")
print(f"Exp34: FIX POOLING - Last Non-Pad Token")
print(f"{'='*70}")
print(f"🔧 FIX: Using attention_mask to find actual last token (not PAD)")
...
# ⭐⭐⭐ FIXED CLASSIFICATION HEAD - USES LAST NON-PAD TOKEN ⭐⭐⭐
class ClassificationHead(nn.Module):
    """Classification head with FIXED pooling - uses last non-pad token"""
    ...
    def forward(self, hidden_states, attention_mask):
        # ⭐ FIX: Get last NON-PAD token instead of last position
        # attention_mask: (B, T) with 1 for real tokens, 0 for PAD
        # Sum gives the count of real tokens, subtract 1 for 0-indexed position
        idx = attention_mask.sum(dim=1) - 1  # (B,)
        idx = idx.clamp(min=0)  # Prevent negative indices
        
        # Gather the hidden state at the last real token position for each batch
        batch_indices = torch.arange(hidden_states.size(0), device=hidden_states.device)
        last_hidden = hidden_states[batch_indices, idx]  # (B, H)
...
class CausalRAG7B(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        # ⭐ Pass attention_mask to classifier for proper pooling
...
def main():
    ...
    print("\n[5/6] Training with FIXED POOLING...")
    ...
    zip_path = create_submission(test_probs, test_ids, ckpt['threshold'], config.OUTPUT_DIR, 'exp34_fix_pooling_submission')
    
    with open(config.OUTPUT_DIR/'results.json', 'w') as f:
        json.dump({
            'experiment': 'exp34_fix_pooling',
            'fix': 'last_non_pad_token_pooling',
    ...
    print(f"\n{'='*70}")
    print(f"🏆 Exp34: FIX POOLING Complete!")
    print(f"  🔧 FIX: Last non-pad token pooling (was using PAD token!)")
    print(f"  Model: {config.MODEL_NAME}")
