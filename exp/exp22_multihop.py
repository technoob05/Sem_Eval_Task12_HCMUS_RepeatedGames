...
"""
================================================================================
Exp22 MultiHop: CausalRAG + Multi-hop Causal Path Retrieval
================================================================================
SELF-CONTAINED - Copy to Kaggle notebook

🎯 TARGET: 0.80+ (from 0.78)
🏆 NOVELTY: Multi-hop Causal Path Retrieval (2-hop traversal)

📚 PAPER CONTRIBUTION:
"We extend CausalRAG with multi-hop causal path traversal, 
enabling deeper reasoning chains like A→B→C instead of just A→B"

✅ SAFE CHANGES ONLY:
- Multi-hop path retrieval (novel, safe)
- Full document content (proven, safe)
- Same BCE loss (no risky multi-task)
- Same training loop (no FGM/SWA/TTA)
...
class Config:
    ...
    if IS_KAGGLE:
        ...
        OUTPUT_DIR = Path('/kaggle/working/exp22mh_output')
    else:
        DATA_DIR = Path(r'd:\Work\SemEval 2026 Task 12 - Abductive Event Reasoning\semeval2026-task12-dataset')
        OUTPUT_DIR = Path(r'd:\Work\SemEval 2026 Task 12 - Abductive Event Reasoning\exp22mh_output')
    
    # Model - use v3-large (stable) first, can upgrade to xlarge later
    MODEL_NAME = 'microsoft/deberta-v3-large'
    MAX_LENGTH = 512
    
    # ⭐ MULTI-HOP CAUSAL SETTINGS
    CAUSAL_PATTERNS = [
        (r'(.+?) caused (.+)', 'CAUSE'),
        (r'(.+?) led to (.+)', 'CAUSE'),
        (r'(.+?) resulted in (.+)', 'CAUSE'),
        ...
        (r'due to (.+?), (.+)', 'CAUSE'),
        (r'(.+?) contributed to (.+)', 'CAUSE'),
    ]
    CAUSAL_EDGE_BOOST = 1.5
    MAX_GRAPH_EDGES = 30
    MAX_HOP = 2  # ⭐ NEW: Multi-hop depth
    PATH_BOOST = 2.0  # ⭐ NEW: Boost for chunks on causal path
    
    # RAG Settings
    CHUNK_SIZE = 350
    TOP_K_CHUNKS = 4  # Increased for multi-hop
    MAX_CONTEXT = 1800  # Increased for richer context
...
print(f"{'='*70}")
print(f"Exp22 MultiHop: CausalRAG + Multi-hop Path Retrieval")
print(f"{'='*70}")
...
# ============================================================================
# ⭐ MULTI-HOP CAUSAL GRAPH BUILDER
# ============================================================================
class MultiHopCausalGraph:
    def __init__(self):
    ...
    def build_graph(self, topic_info):
        """Build adjacency list for multi-hop traversal"""
        if not topic_info:
            return {'nodes': set(), 'edges': [], 'adj': defaultdict(list)}
        ...
        # Build adjacency list: cause -> [effects]
        ...
        return {'nodes': nodes, 'edges': all_edges, 'adj': dict(adj)}
    
    def find_multihop_paths(self, graph, start_terms, max_hop=2):
        """
        ⭐ NOVELTY: Find all nodes reachable within max_hop hops
        Returns: set of all reachable nodes and the paths
        """
        adj = graph.get('adj', {})
        nodes = graph.get('nodes', set())
        
        # Find starting nodes that match any start term
        start_nodes = set()
        for term in start_terms:
            term_lower = term.lower()
            for node in nodes:
                if term_lower in node or node in term_lower:
        ...
        current_level = list(start_nodes)
...
causal_graph = MultiHopCausalGraph()
...
class MultiHopRetriever:
    ...
    def retrieve(self, topic_info, event, options, max_chars=1800):
        ...
        # Build causal graph
        graph = causal_graph.build_graph(topic_info)
        
        # ⭐ Find multi-hop reachable nodes from query terms
        query_terms = event.split() + [opt.split()[0] for opt in options if opt]
        reachable_nodes, paths = causal_graph.find_multihop_paths(
            graph, query_terms, max_hop=config.MAX_HOP
        )
        ...
        for doc in topic_info.get('docs', []):
            ...
            for i in range(0, len(content), config.CHUNK_SIZE - 50):
                ...
                # Check if chunk contains any causal edge
                has_causal = any(
                    e['cause'] in chunk_lower or e['effect'] in chunk_lower
                    for e in graph['edges']
                )
                
                # ⭐ Check if chunk is on multi-hop path
                on_path = any(
                    node in chunk_lower for node in reachable_nodes
                )
        ...
        if not chunks:
            return f"Topic: {topic}", {'paths': paths}
        ...
        for i, meta in enumerate(chunk_meta):
            ...
            if meta['on_path']:  # ⭐ Multi-hop path boost
                sims[i] *= config.PATH_BOOST
        ...
        causal_summary = ""
        if graph['edges']:
            edge_strs = [f"'{e['cause'][:20]}'→'{e['effect'][:20]}'" for e in graph['edges'][:3]]
            causal_summary = f"\nCausal chains: {'; '.join(edge_strs)}"
        
        # ⭐ Add multi-hop path info
        if paths:
            path_summary = f"\nMulti-hop: {'; '.join(paths[:3])}"
        else:
            path_summary = ""
        
        context = f"Topic: {topic}{causal_summary}{path_summary}\n\nEvidence:\n" + "\n---\n".join(selected)
        return context[:max_chars], {'paths': paths, 'reachable': len(reachable_nodes)}
...
def main():
    ...
    print("\n[2/6] Initializing Multi-Hop Retriever...")
    retriever._load()
    ...
    print("\n[5/6] Training MultiHop CausalRAG...")
    ...
    zip_path = create_submission(test_probs, test_ids, ckpt['threshold'], config.OUTPUT_DIR, 'exp22mh_submission')
    ...
    with open(config.OUTPUT_DIR/'results.json', 'w') as f:
        json.dump({
            ...
            'novelty': 'Multi-hop causal path retrieval',
            'baseline_exp22': 0.78
        }, f, indent=2)
    
    print(f"\n{'='*70}")
    print(f"🏆 MultiHop CausalRAG Complete!")
    print(f"  Model: {config.MODEL_NAME}")
    print(f"  Max Hop: {config.MAX_HOP}")
