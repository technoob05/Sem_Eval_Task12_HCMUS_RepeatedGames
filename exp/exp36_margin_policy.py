...
"""
================================================================================
Exp36: MARGIN POLICY - Top-1/Top-2 Margin-Based Inference
================================================================================
SELF-CONTAINED - Copy to Kaggle notebook

🎯 KEY INSIGHT: AER metric gives 0.5 for subsets
📊 Current threshold sweep only considers probability threshold
✅ SOLUTION: Margin-based policy - select top-2 if p2 is close to p1

Policy:
- Default: select top-1
- If p2 >= p1 - margin: select {top1, top2}
- Grid search margin on dev set
...
class Config:
    ...
    if IS_KAGGLE:
        ...
        OUTPUT_DIR = Path('/kaggle/working/exp36_margin_policy_output')
    else:
        DATA_DIR = Path(r'd:\Work\SemEval 2026 Task 12 - Abductive Event Reasoning\semeval2026-task12-dataset')
        OUTPUT_DIR = Path(r'd:\Work\SemEval 2026 Task 12 - Abductive Event Reasoning\exp36_margin_policy_output')
    ...
    # ⭐ Margin Policy Settings
    MARGIN_RANGE = np.arange(0.0, 0.35, 0.025)  # Search margins from 0 to 0.35
    MAX_K_OPTIONS = [1, 2, 3]  # Max options to select
...
print(f"{'='*70}")
print(f"Exp36: MARGIN POLICY - Top-1/Top-2 Inference")
print(f"{'='*70}")
print(f"✅ Margin-based selection: top-2 if p2 close to p1")
print(f"✅ Grid search margin and max_k")
...
# ============================================================================
# ⭐ MARGIN-BASED INFERENCE POLICY
# ============================================================================
def margin_policy_decode(probs, margin, max_k):
    """
    Decode predictions using margin policy
    
    Args:
        probs: (N, 4) probability array
        margin: threshold for selecting multiple options
        max_k: maximum number of options to select
    ...
    for p in probs:
        ...
        # Check if additional options should be selected based on margin
        for i in range(1, min(max_k, 4)):
            if i < len(sorted_probs):
                # Select if within margin of top-1
                if sorted_probs[i] >= sorted_probs[0] - margin:
                    selected.append(sorted_idx[i])
                else:
                    break  # Stop if gap too large
        
        # Convert to answer string
        answer = ','.join(sorted([opts[i] for i in selected]))
        predictions.append(answer)
    
    return predictions

def optimize_margin_policy(probs, golds):
    """Grid search for best margin and max_k combination"""
    best_score = 0
    best_margin = 0
    best_max_k = 1
    ...
    for max_k in config.MAX_K_OPTIONS:
        for margin in config.MARGIN_RANGE:
            preds = margin_policy_decode(probs, margin, max_k)
            score = compute_aer_score(preds, golds)
            results.append((margin, max_k, score))
    ...
    return best_margin, best_max_k, best_score, results
...
def main():
    ...
    print("\n[5/6] Training with Margin Policy optimization...")
    ...
    for epoch in range(1, config.EPOCHS + 1):
        ...
        # ⭐ Use margin policy optimization instead of threshold sweep
        margin, max_k, score, search_results = optimize_margin_policy(dev_probs, dev_gold)
        print(f"  Epoch {epoch}: Loss={train_loss:.4f}, Dev={score:.4f} (margin={margin:.3f}, max_k={max_k})")
    ...
    print(f"  Best Margin: {best_margin:.3f}")
    print(f"  Best Max K: {best_max_k}")
    ...
    test_answers = margin_policy_decode(test_probs, ckpt['margin'], ckpt['max_k'])
    zip_path = create_submission(test_answers, test_ids, config.OUTPUT_DIR, 'exp36_margin_policy_submission')
    
    with open(config.OUTPUT_DIR/'results.json', 'w') as f:
        json.dump({
            'experiment': 'exp36_margin_policy',
            'method': 'margin_based_top_k_selection',
            'dev_score': float(best_score),
            'best_margin': float(best_margin),
            'best_max_k': int(best_max_k),
            'model': config.MODEL_NAME,
            'baseline_exp22': 0.78
        }, f, indent=2)
    
    print(f"\n{'='*70}")
    print(f"🏆 Exp36: MARGIN POLICY Complete!")
    print(f"  📊 Margin-based top-1/top-2 selection")
    print(f"  ✅ Includes pooling fix")
    print(f"  Best Dev: {best_score:.4f}")
    print(f"  Best Margin: {best_margin:.3f}, Max K: {best_max_k}")
