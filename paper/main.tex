...
\begin{tikzpicture}[
    ...
    font=\small\sffamily,
    block/.style={rectangle, draw=gray!60, fill=white, text width=2.2cm, text centered, minimum height=1.2cm, rounded corners=4pt, line width=1pt, drop shadow={opacity=0.1, shadow xshift=1pt, shadow yshift=-1pt}},
    process/.style={rectangle, draw=blue!60!black, fill=blue!5, text width=2.5cm, text centered, minimum height=1.2cm, rounded corners=4pt, line width=1pt, drop shadow={opacity=0.1, shadow xshift=1pt, shadow yshift=-1pt}},
    model/.style={rectangle, draw=purple!60!black, fill=purple!5, text width=2.8cm, text centered, minimum height=1.4cm, rounded corners=4pt, line width=1.2pt, drop shadow={opacity=0.15, shadow xshift=1pt, shadow yshift=-1pt}},
    arrow/.style={->, >=stealth, thick, color=gray!70!black},
    group/.style={rectangle, draw=gray!40, dashed, inner sep=0.4cm, rounded corners=6pt, fill=white}
]

% === INPUTS ===
\node[block] (docs) {\textbf{Retrieved}\\Documents};
\node[block, below=1.2cm of docs] (query) {\textbf{Input}\\Event + Options};
...
% === FUSION ===
\node[process, right=2cm of search, yshift=1.2cm, text width=2.2cm] (fusion) {Evidence\\Ranking \& Selection};

% === GENERATION MODULE ===
\node[model, right=2cm of fusion] (llm) {\textbf{Qwen2.5-32B}\\\textit{Extended LoRA}};
\node[process, right=1.2cm of llm, text width=2cm] (head) {Multi-Label\\Classifier};
\node[block, right=1.2cm of head, fill=green!5, draw=green!60!black] (output) {\textbf{Answer}\\Predictions};

% === GROUPS ===
\begin{pgfonlayer}{background}
    \node[group, fill=gray!5, fit=(graph) (search) (fusion), label={[anchor=north west, color=gray!80!black, font=\scriptsize\bfseries]north west:Retrieval System}] (retrieval_group) {};
    \node[group, fill=gray!5, fit=(llm) (head), label={[anchor=north west, color=gray!80!black, font=\scriptsize\bfseries]north west:Reasoning System}] (reasoning_group) {};
\end{pgfonlayer}

% === CONNECTIONS ===
% Docs Flow
\draw[arrow] (docs) -- node[above, font=\scriptsize, color=gray!80!black] {Context} (graph);
\draw[arrow] (graph) -| node[pos=0.25, above, font=\scriptsize, color=red!70!black] {+Causal Boost} (fusion);
...
% Fusion to Model
\draw[arrow] (fusion) -- node[above, font=\scriptsize, text width=1.5cm, align=center] {Top-3\\Chunks} (llm);
...
\node[above=0.1cm of search, font=\scriptsize, color=gray!80!black] {$\alpha$=0.3 (BM25)};
\node[below=0.1cm of search, font=\scriptsize, color=gray!80!black] {Score Fusion};
