...
\title{CausalRAG at SemEval-2026 Task 12: Hybrid Retrieval and Extended LoRA for Abductive Event Reasoning}
...
In stark contrast to the extensive research on natural language inference \cite{devlin2019bert,liu2019roberta} and question answering \cite{rajpurkar2016squad}, exploration of abductive reasoning in the context of real-world events lags behind, mainly due to the lack of large-scale datasets. To close this gap, SemEval-2026 Task 12: Abductive Event Reasoning (AER) is proposed to encourage research on causal reasoning with document evidence. The task presents a realistic scenario: given an event and retrieved documents as background, systems must identify which of four candidate explanations most plausibly caused the event.
...
\end{equation}
where $\hat{s}$ denotes normalized scores and $\alpha = 0.3$ in our experiments. BM25 \cite{robertson2009probabilistic} provides fast lexical matching that captures explicit causal keywords, while dense retrieval using Sentence-BERT \cite{reimers2019sentence} captures semantic similarity beyond lexical overlap. Documents are chunked into segments of approximately 300 characters with 50-character overlap, and we retrieve the top-$k$ chunks (with $k=3$).

\subsection{Extended LoRA Fine-tuning}
\label{sec:lora}

We fine-tune Qwen2.5-32B-Instruct \cite{qwen2024} using QLoRA \cite{dettmers2023qlora} with 4-bit quantization. Standard LoRA \cite{hu2022lora} adapts pre-trained models by adding low-rank decomposition matrices to the attention layers while keeping the original weights frozen. This approach dramatically reduces the number of trainable parameters while maintaining competitive performance.
...
Table~\ref{tab:leaderboard} presents the official competition leaderboard at the time of submission. Our system (HCMUS\_TheFangs) ranks \textbf{first} among all 19 participating teams, achieving the highest score of 0.90.
...
\midrule
\textbf{1} & \textbf{HCMUS\_TheFangs (Ours)} & \textbf{0.90} \\
2 & zayme & 0.89 \\
3 & songohan & 0.88 \\
4 & Winner Here & 0.88 \\
5 & dmig\_rorofan & 0.84 \\
6 & uit\_nkn & 0.82 \\
7 & baotuan & 0.82 \\
8 & chisboizhoigay & 0.78 \\
9 & dorriiii & 0.74 \\
10 & wuhjie & 0.71 \\
\midrule
\multicolumn{3}{l}{\textit{... 9 more teams (0.70--0.30)}} \\
\bottomrule
\end{tabular}
\caption{Official leaderboard (top 10 of 19 teams). Our system achieves the highest score.}
